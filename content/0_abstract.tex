...
Diese Tests sollen auf unterschiedlichen Architekturen laufen können. Es ist schwierig, Software für grundlegende Hardware ohne Zugriff darauf zu testen. Deshalb muss die in IBM Z verwendete s390x-Architektur für ausgewählte Open-Source-Projekte emuliert (nachgeahmt) und in die entsprechende \gls{CI/CD}-Pipeline eingefügt werden. 
Das sollte mit schnellen Deployment-Methoden im Emulator \gls{QEMU} laufen. 
...
Zusätzlich müssen die minimalen Systemanforderungen für die Einrichtung innerhalb der \gls{CI/CD}-Infrastruktur beider Projekte wegen der Minimierung an Festplattenplatz, Memory und CPU-Verbrauch analysiert werden. \\
...
These tests should be able to run on different architectures. It is difficult to test software for essential hardware without access. Therefore, the \gls{s390x} architecture used in IBM Z has to be emulated on x86 for chosen open-source projects and included in their \gls{\gls{CI/CD}} pipeline. 
That should run with fast deployment methods in the \gls{emulator} \gls{QEMU}. 
...
Additionally, minimal system requirements have to be analyzed for the setup inside of the \gls{CI/CD} infrastructure of both projects concerning the minimization of disk space, memory and CPU usage for deployments. \\
