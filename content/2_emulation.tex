...
QEMU is an open-source emulator introduced by Fabrice Bellard in the year 2003 and available in most Linux distributions now. It is embedded in different virtualization tools as KVM and XEN, too. So QEMU is well tested and has got all the necessary features for emulations. 
You can emulate other architectures on different hardware. QEMU is a generic emulator and can emulate different system architectures. It can also be used for emulation of obsolete hardware \cite[~p.24]{Opsahl2013}. 
...
The System Emulation emulates a whole system with hardware, the operating system (with the kernel) and the userspace (with application processes). 
...
You can register other architectures within that, so multiple other architectures can be run on this host. 
...
The installation contains emulators for all available architectures supported by the QEMU project incl. s390x aligned with the specific host architecture. In this manner described, you can build and emulate for different architectures on the same host (see \ref{Optimized-Qemu-Command} Optimized Qemu Command). That is the base for BuildX by Docker, too.
...
So it would be nice to integrate the docker file system into QEMU. After building a docker image, it is realizable to export the file system into a directory with the name \textbf{rootfs} with the command \\ 
...
\lstinline!qemu-img create -f raw cassandra.img 2G! (see \ref{Cassandra-Deployment}Cassandra Deployment) \\
