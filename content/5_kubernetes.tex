...
The base image for a specified architecture can be integrated with 2 methods. Firstly, the s390x base image can be pulled to the local container registry with \textbf{docker pull s390x/docker}. The specification \textbf{docker} alone would download the Docker image for the host architecture. The prefix \textbf{s390x} before docker specifies a specific available architecture. 
