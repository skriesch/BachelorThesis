  ...
  prepare-test:
    ...
    steps:
      ...
      - run: 
            ...
            command: docker build buildx --platform=linux/s390x --squash \
                     --build-arg -t cassandra-test-s390x .
      - docker/check:
         registry: $DOCKER_REGISTRY 
    ...
    steps:
      ...
      - run:
             ...
             command: |
                     ...
                     -m 48 -M s390-ccw-virtio -nodefaults -device sclpconsole,\
...
\begin{boxedverbatim}
agent {
    // "docker build -f Dockerfile.alpine --build-arg version=${KUB_VERSION} ./s390x-kubernetes-test/
    dockerfile {
        filename 'Dockerfile.alpine'
        dir 's390x-kubernetes-test'
        label 'kubernetes'
        additionalBuildArgs  '--build-arg version=${KUB_VERSION}'
        args '-v /tmp:/tmp'
    }
}

agent {
    // "docker build -f Dockerfile.alpine-test --build-arg version=${KUB_VERSION} ./s390x-kubernetes-test/test/
    dockerfile {
